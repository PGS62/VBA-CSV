...
'' Purpose    : Export the modules of this workbook to the vba folder, also save the workbook in its current location,
...
'
'    Set wb = ThisWorkbook
'    Folder = Left$(ThisWorkbook.path, InStrRev(ThisWorkbook.path, "\")) + "vba\" & wb.Name
'
...
'    SubFolder = Left(wb.Name, InStrRev(wb.Name, ".") - 1) & "_Backups"
'
'    BackUpBookName = Environ$("OneDriveConsumer") + "\Excel Sheets\" & SubFolder & "\" + Replace(ThisWorkbook.Name, ".", "_v" & shAudit.Range("B6") & ".")
'
